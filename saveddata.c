...
void saveCardData(char *inputcarddata, int inputlength, uint8_t positiontosaveto){
    if(positiontosaveto>numberOfCardDataSaveSlots-1)return;
    
    uint8_t bytetowrite=0;
    uint8_t dataoffset=0;
    //pack two data characters into each byte before writing. Low bits even characters then high bits odd characters.
...
uint8_t retrieveCardData(char *buffer, uint8_t positiontoreadfrom){
    if(positiontoreadfrom>numberOfCardDataSaveSlots-1)return 0;
